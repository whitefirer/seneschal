...
// MakeTriggerCallback creates a TriggerFunc that starts the runbook's
// workflow through the shared execution pipeline (startRun). The execution ID
// is generated by that pipeline and returned synchronously; the run itself is
// asynchronous and gets the same in-memory record, WebSocket progress, and
// snapshot persistence as any other execution.
//
...
func MakeTriggerCallback(hub *WSHub, workflowsDir string, startRun func(wf *workflow.Workflow, name, path string, vars map[string]string) (string, error), aiCfg workflow.AIConfig) workflow.TriggerFunc {
	return func(rb *workflow.RunbookConfig, extraVars map[string]string) (string, error) {
		...
		// Start through the shared pipeline (async, WS progress, persisted).
		execID, err := startRun(wf, filepath.Base(wfPath), wfPath, vars)
		if err != nil {
			broadcast("", "", "failed", err.Error())
			return "", fmt.Errorf("runbook %s: start workflow: %w", rb.Name, err)
		}
