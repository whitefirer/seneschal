...
import { Trash2, Plus, ChevronDown, ChevronRight, GitBranch, Layers, Repeat, Terminal } from 'lucide-react'
...
function containerBadge(action: string): { icon: any; label: string } | null {
...
function FieldEditor({ field, value, onChange }: {
  ...
  if (field.type === 'select') {
    return (
      <select value={value ?? ''} onChange={(e) => onChange(e.target.value)} className="w-full px-2 py-1 text-xs border rounded bg-background">
        <option value="">—</option>
...
// key-value 编辑器
function KVEditor({ value, onChange }: { value: Record<string, string>; onChange: (v: any) => void }) {
...
function ListEditor({ value, onChange, allowString }: { value: any; onChange: (v: any) => void; allowString?: boolean }) {
      ...
      <div className="px-2 py-2 space-y-1.5">
        ...
        {def?.fields.map((f) => (
          <div key={f.key}>
            ...
            <FieldEditor field={f} value={data[f.key]} onChange={(v) => patch(f.key, v)} />
          </div>
        ))}
